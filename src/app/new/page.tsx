...
import { mergeCase, flagProbable } from "@/lib/ops";
...
export default function NewCasePage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Offline: the case was queued locally; SyncManager will flush it.
  ...
  if (phase === "done" && offline) {
    return (
      <div className="max-w-2xl space-y-4">
        <div className="nb-card bg-[var(--warning)] p-5">
          <div className="nb-label">Saved offline</div>
          <p className="font-bold mt-1">
            No connection — this case is queued on this device and will sync
            automatically when you&apos;re back online (duplicate check + share link
            issue on sync).
          </p>
        </div>
...
function DoneCard({
  ...
  async function flagProbableMatch(matchId: string) {
    await flagProbable(result.id, matchId);
    setProbable(matchId);
  }
              ...
              <li key={c.caseId} className="py-2 flex items-center justify-between gap-2 text-sm font-bold">
                ...
                <span className="flex items-center gap-2">
                  ...
                  {merged === c.caseId ? (
                    ...
                    <span className="nb-chip bg-[var(--cyan)]">probable</span>
                  ) : (
                    <>
                      <button
                        type="button"
                      ...
                      <button
                        type="button"
                        onClick={() => flagProbableMatch(c.caseId)}
                        disabled={merged !== null || probable !== null}
                        className="nb-btn text-[10px] py-0.5 px-2"
                      >
                        Probable
                      </button>
                    </>
